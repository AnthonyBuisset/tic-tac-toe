...
impl TicTacToeContract {
    pub fn create_game(env: Env, player_x: Symbol, player_o: Symbol) -> u32 {
        let game_counter = env.storage().persistent().get(&DataKey::GameCounter).unwrap_or(0u32);
        ...
        env.storage().persistent().set(&DataKey::Game(new_game_id), &game);
        env.storage().persistent().set(&DataKey::GameCounter, &new_game_id);
    ...
    pub fn make_move(env: Env, game_id: u32, player: Symbol, position: u32) -> Game {
        ...
        let mut game: Game = env.storage().persistent().get(&DataKey::Game(game_id))
    ...
    pub fn get_board(env: Env, game_id: u32) -> Vec<Option<Player>> {
        let game: Game = env.storage().persistent().get(&DataKey::Game(game_id))
            .expect("Game not found");
        game.board
    }
